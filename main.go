...
type VisitMessage struct {
	UserProfileLinkId string `json:"userProfileLinkId"`
...
func processMessage(db *sql.DB, message events.SQSMessage) {
	...
	fmt.Printf("User Profile Link id is %s \n", data.UserProfileLinkId)
...
func insertVisit(db *sql.DB, data VisitMessage) {
	...
	defer cancelfunc()
	insertSql := "INSERT INTO UserProfileLinkVisit(id, userProfileLinkId, visitMetadata) VALUES(?, ?, ?)"
	_, err := db.ExecContext(ctx, insertSql, uuid.New(), data.UserProfileLinkId, data.VisitMetadata)
	if err != nil {
		log.Fatal(err)
